...
export async  function historyAnalysisLoader({params}) {
    const {siteID, meterID} = params
    const lastDayStr = dayjs().subtract(1, 'd').format('YYYY-MM-DD');

    const eneryData = await getLatestEnerydataById(siteID, meterID)
    const lastDayData = await fetchData(`/api/datas/collectdatas/sometime/${siteID}`); // pue Data
...
function HistoryAnalysis() {
    ...
    const [uiState, setUiState] = useState(
        {
                    displaySearchTableBar: true,
                    timeTitle: stationData.lastDayData.timeStr,
                    timeRadioValue: 'date'
                }
        )
    ...
    const timePickerChange = async (date, dateString) => {
        ...
        const newPueData = await fetchData(
            `/api/datas/collectdatas/sometime/${siteID}/${dtypeDic[uiState.timeRadioValue]}/${dateString}`);
        setPueData(pueData => { return {...pueData, realTimeData: newPueData}});
        setUiState(uiState => {return {...uiState, timeTitle: newPueData.timeStr}})
        /* 请求 TopMiddleDiagram 数据 */
        const currentSeries = await fetchData(`/api/datas/collectdatas/currentseriesdata/${siteID}/${dtypeDic[uiState.timeRadioValue]}/${dateString}`);
        const temperatureSeries = new Array(currentSeries.timeSeries.length).fill(0)
        ...
        <div id="select-group-channel-tablebar"
            ...
            <div className="row">
                <div className="col-sm-2  col-md-2 pd"
                     style={{
                         color: '#fff', height: '40px', lineHeight: '35px', marginTop: '15px',
                         textAlign: 'right', width: '110px', fontSize: '15px'
                     }}>类型选择：
                </div>
                <div className="col-sm-3  col-md-3 pd" style={{height: '40px', marginTop: '15px'}}>
                    <Radio.Group options={[{label: '日分析', value: 'date'}, {label: "月分析", value: 'month'}]}
                                 value={uiState.timeRadioValue}
                                 onChange={({target: {value}}) => setUiState({...uiState, timeRadioValue: value})}
                                 optionType='button'
                                 buttonStyle='solid'/>
                </div>
                <div className="col-sm-2  col-md-2 pd"
                    ...
                    <div className="col-sm-7  col-md-7 pd analysis-filter">
                        <table style={{width: '100%', height: '48px'}}>
                            ...
                            <tr>
                                ...
                                <td style={{width: '15%'}}>
                                    <div className="cust-type-default right active" type="2">历史分析</div>
                                    <div className="cust-type-default left"
                                         onClick={() => navigate(`/station/${siteID}/${meterID}`)} type="1">实时监测</div>
                                </td>
                            </tr>
                            </tbody>
                        </table>
