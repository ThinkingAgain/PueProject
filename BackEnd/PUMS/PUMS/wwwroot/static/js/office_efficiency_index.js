...
function loadChannelBaseInfo(data){
	...
	$("#base-info .device-score").find(".score-val").text("5434度");
	
	//设置渠道当日客流量、订单量
	channelDeviceInfo.channelOrderNum = "5806度"
	channelDeviceInfo.channelCustNum = "5342元"
	...
	$("#base-info .weigui").find(".score-val").text("4807度");
	//设置门店基本信息-营销评级
	...
	$("#base-info .yingxiao").find(".score-val").text("534度");
	//设置门店基本信息-受理速度
	var avgTimeScore=0;
	...
	$("#base-info .avgtime").find(".score-val").text("187.31度");
	//设置门店基本信息-业务量
	var yewuliangVal=0;
	...
	$("#base-info .yewuliang").find(".score-val").text("31天");
	//设置门店基本信息-排队机耗时
	var channelHandleInfo=data.channelHandleInfo;
    ...
    // 修改电费信息下一行的显示 --- begin
	g_channelName = "报账月份: 2024年2月 (2024.1.15-2024.2.15)"
	// 修改电费信息下一行的显示 --- end
	...
	$("#base-info").find(".popval").text("超标43.29%" || "全省前0%");
    var rePortData={};
    //厅办理业务平均耗时
...
function channelReportContent(rePortData){
	...
	reportContent = "能耗预警: 当前异常电量超标, 超标准电量43.29% "
...
function loadChannelHandleDetail(data){
	var option = {
		    ...
		    yAxis: [{
		        ...
		        axisLine: {lineStyle: {color: '#57617B'}},
		        axisLabel: {margin: 10,textStyle: {fontSize: 12},textStyle: {color:'#fff'},formatter:'{value}A'},
	...
	for(var key in staffHandleInfo){
		...
		var tr1=[
			...
			'<td colspan="3">',
					...
					'<div class="avg-time-label">|&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;我方</div>',
					'<div class="avg-time-value">'+itemArr[0].unicom+'</div>',
					'<div class="avg-time-label">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;移动</div>',
					'<div class="avg-time-value">'+itemArr[0].mobil+'</div>',
					'<div class="avg-time-label">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;联通</div>',
					'<div class="avg-time-value">'+itemArr[0].tele+'</div>',
				'</td>',
			'</tr>',
			'<tr>',
				'<td> ',
					'<div class="staff-cust-time">',
					   '<span style="font-size:15px;">'+itemArr[0].powerTypeLabel+'</span><br/><span style="color:#00A8FE;font-size:18px;font-weight:600;">'+itemArr[0].powerType+'</span>',
					'</div>',
				'</td>',
				'<td>',
					'<div class="staff-order-count">',
					    '<span style="font-size:15px;">'+itemArr[0].siteTypeLabel+'</span><br/><span style="color:#00A8FE;font-size:18px;font-weight:600;">'+itemArr[0].siteType+'</span>',
					'</div>',
				'</td>',
				'<td>',
					'<div class="staff-alarm">',
					   '<span style="font-size:15px;">'+itemArr[0].ownerInfoLabel+'</span><br/><span style="color:#00A8FE;font-size:18px;font-weight:600;">'+itemArr[0].ownerInfo+'</span>',
					'</div>',
			    '</td>',
			'</tr> ',
			'<tr class="td-integral"> ',
				'<td colspan="3"> ',
				'<div class="integral-label">站点ID：</div> ',
				'<div class="integral-value">'+itemArr[0].siteId+'</div> ',
				'<div class="integral-label" style="width:60px;">电表ID:</div>',
				'<div class="integral-value" style="width:100px;">'+itemArr[0].meterId+'</div>',
				'</td> ',
			'</tr> ',
		];
		var tr2=[];
			...
			tr2=[
				...
				'<tr class="td-avg-time"> ',
						'<td colspan="3"> ',
							...
							'<div class="avg-time-label">|&nbsp;&nbsp;'+itemArr[1].siteName+'</div>',
				...
				'<tr>',
						'<td> ',
							'<div class="staff-cust-time">',
							   '<span style="font-size:15px;">'+itemArr[1].departmentLabel+'</span><br/><span style="color:#00A8FE;font-size:18px;font-weight:600;">'+itemArr[1].department+'</span>',
							'</div>',
						'</td>',
						'<td>',
							'<div class="staff-order-count">',
							    '<span style="font-size:15px;">'+itemArr[1].maintainerLabel+'</span><br/><span style="color:#00A8FE;font-size:18px;font-weight:600;">'+itemArr[1].maintainer+'</span>',
							'</div>',
						'</td>',
						'<td>',
							'<div class="staff-alarm">',
							   '<span style="font-size:15px;">'+itemArr[1].mphoneLabel+'</span><br/><span style="color:#00A8FE;font-size:18px;font-weight:600;">'+itemArr[1].mphone+'</span>',
							'</div>',
					    '</td>',
				'</tr> ',
				'<tr class="td-integral"> ',
					'<td colspan="3"> ',
					'<div class="integral-label">站址编码：</div> ',
					'<div class="integral-value">'+itemArr[1].siteCode+'</div> ',
					'<div class="integral-label" style="width:60px;">能耗状态: </div>',
					'<div class="integral-value" style="width:100px;">&nbsp;&nbsp;'+itemArr[1].powerStatus+'</div>',
					'</td> ',
			   '</tr> ',
			]
		}
	...
	for(var key in deviceInfo){
		...
		var tr1=[
				'<tr> ',                                                            
					...
					'</td> ',                                                           
				    '<td><div class="label-name"><b>P • U • E</b></div></td> ',
				    '<td rowspan="3" colspan="2" class="kuan-dai"> ',                   
				            '<div class="progress-label">机房配电输入电流 '+itemArr[0].stationCurrent+'</div>',
							'<div style="width:180px;height:15px;"> ',                         
								'<div class="progress" style="float:left;height:8px;width:180px;background-color:#383E60;">',
								     '<div class="progress-bar progress-bar-striped" style="min-width:0;width:'+'100%"></div>',
								'</div>',                                                    
							'</div> ',                                                      
				            '<div class="progress-label">开关电源输入电流 '+itemArr[0].deviceCurrent+'</div>',
							'<div style="width:180px;height:15px;">',                      
								'<div class="progress" style="float:left;height:8px;width:180px;background-color:#383E60;">',
								   '<div class="progress-bar progress-bar-striped" style="min-width:0;width:'+'66.1562%"></div>',
								'</div>',                                                    
							'</div>',                                                      
				     '</td>',                                                           
				 '</tr>',                                                           
				 '<tr>',                                                            
				     '<td class="label-name score">'+itemArr[0].pue+'</td>',
				 '</tr>',                                                           
				 '<tr>',                                                            
				 ...
				 '<tr class="device-use">',                                         
						'<td></td>',                                                       
						'<td><div class="nei-cun-size"><span>'+(itemArr[0].dcv || 0)+'</span></div></td>',
						'<td><div class="cpu-use"><span>'+itemArr[0].loadCurrent+'</span></div></td>',
						'<td><div class="nei-cun-use"><span>'+itemArr[0].airConditionerStatus+'</span></div></td>',
				'</tr>',                                                           
				'<tr>',                                                            
						'<td></td>',                                                       
						'<td><div class="labe-value">&nbsp;&nbsp;直 流 电 压</div></td>',
						'<td><div class="labe-value">&nbsp;&nbsp;负 载 电 流</div></td>',
						'<td><div class="labe-value">&nbsp;&nbsp;空 调 状 态</div></td>',
				'</tr>',
			'<tr class="device-use">',
				'<td></td>',
				'<td><div class="nei-cun-size"><span>'+(itemArr[0].temperature || 0)+'</span></div></td>',
				'<td><div class="cpu-use"><span>'+itemArr[0].humidity+'</span></div></td>',
				'<td><div class="nei-cun-use"><span>'+itemArr[0].airConfidtrionerTemperature+'</span></div></td>',
			'</tr>',
			'<tr>',
				'<td></td>',
				'<td><div class="labe-value">&nbsp;&nbsp;机 房 温 度</div></td>',
				'<td><div class="labe-value">&nbsp;&nbsp;机 房 湿 度</div></td>',
				'<td><div class="labe-value">&nbsp;&nbsp;回 风 温 度</div></td>',
			'</tr>',
		];
		var tr2=[];
		if(itemArr.length>1){
	...
	var option = {
		    ...
		    yAxis: [{
        		type: 'value',
 		        ...
 		        axisLine: {lineStyle: {color: '#57617B'}},
 		        axisLabel: {margin: 0,textStyle: {fontSize: 12},textStyle: {color:'#fff'},formatter:'{value}时'},
 		        splitLine: {
 	                show: true,
 	                lineStyle:{
 	                    type:'dashed',
            ...
            yAxis:[
       	        {
       	          type: 'value',name: '',
       	          axisLine: {lineStyle: {color: '#57617B'}},
  		          axisLabel: {margin: 10,textStyle: {fontSize: 12},textStyle: {color:'#fff'},formatter:'{value}度'},
...
function initBaseInfo(){
   ...
   var dataDate="2024年2月22日";
   // 增加日期自动生成---begin
	var date = new Date();
	var year = date.getFullYear(); //当前年份
	var month = date.getMonth(); //当前月份
	var data = date.getDate(); //天
	dataDate = year + "年" + (month + 1) + "月" + data + "日"
	// 增加日期自动生成 --- end
